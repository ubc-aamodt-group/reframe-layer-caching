...
class ExtraNet(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, bilinear=False, skip=True, use_reframe=False):
    ...
    def forward(self, x, feature, mask, hisBuffer, refresh_cache=True):
        ...
        if self.reframe_cache is not None and not refresh_cache:
            # Use cached features
            res = self.up3(self.reframe_cache, x1)
            ...
            if self.use_reframe and refresh_cache:
                # Refresh the cache
                self.reframe_cache = res.clone().detach()
