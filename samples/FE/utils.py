...
def ImgWrite(mPath,prefix,idx,img):
    ...
    # Convert img to 8-bit and save as png
    tonemap = cv.createTonemapDrago(gamma=2.5, saturation=1.0)
    ...
    exr_image_normalized = cv.normalize(img, None, alpha=1, beta=255, norm_type=cv.NORM_MINMAX)
    
    # Convert to 8-bit (required for PNG format)
    exr_image_8bit = np.uint8(exr_image_normalized)
    
    # Save as PNG
    cv.imwrite(os.path.join(mPath,prefix+"."+str(idx).zfill(4)+".png"), exr_image_8bit)
