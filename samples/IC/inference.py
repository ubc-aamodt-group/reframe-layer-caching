...
def main(opts):
    ...
    # > REFRAME start
    print(f"Reframe: {opts.use_reframe}")
    print(f"Reframe: {opts.reframe_cache_policy}")

    model.use_cache = opts.use_reframe
    model.reframe_cache_policy = opts.reframe_cache_policy
    # > REFRAME end
    ...
    for scan in tqdm(scans, desc="Looping over scans"):
        ...
        # > REFRAME start
        reframe_skipped = model.reframe_skipped
        print(f"Skipped frames: {reframe_skipped}")
        
        # Ignore the first one
        timer = timer[1:]
        print(f"Total inference time: {sum(timer):.3f}")
        print(f"Mean inference time: {np.mean(timer):.3f}")
        # > REFRAME end
