...
# > REFRAME start
def cache_encoder_forward(self, x, use_reframe=False):
...
class BDModel(pl.LightningModule):
    def __init__(self, opts):
        ...
        # > REFRAME start
        self.use_reframe = False
        self.reframe_cache = None
        self.reframe_cache_policy = None
        self.reframe_cached_image = None
        self.frame_counter = 0
        self.reframe_skipped = 0
        # > REFRAME end
        ...
        if "efficientnet" in self.run_opts.image_encoder_name:
            ...
            # > REFRAME start
            # Overwrite the forward method to include the cache
            global original_forward
            original_forward = self.encoder.forward
            self.encoder.forward = types.MethodType(cache_encoder_forward, self.encoder)
            # > REFRAME end
        ...
        if self.run_opts.depth_decoder_name == "unet_pp":
            self.depth_decoder = BDDecoderPP(dec_num_input_ch, use_reframe=self.use_reframe)
    ...
    def forward(
        ...
        if self.use_reframe:
            refresh_cache = True
            if self.reframe_cached_image is not None:
                if "delta" in self.reframe_cache_policy:
                    diff = torch.abs(cur_image - self.cached_image).mean().item()
                    smape = 2 * diff / (torch.abs(cur_image) + torch.abs(self.cached_image)).mean().item()
                    threshold = float(self.reframe_cache_policy.split("-")[1])
                    if smape > threshold:
                        refresh_cache = True
                        self.reframe_cached_image = cur_image
                    else:
                        refresh_cache = False

                elif "N" in self.reframe_cache_policy:
                    every_n = int(self.reframe_cache_policy.split("-")[1])
                    refresh_cache = (self.frame_counter % every_n == 0)
        ...
        # > REFRAME start
        if self.use_reframe:
            if self.reframe_cache is None or refresh_cache:
                cur_feats = self.encoder(cur_image)
                self.reframe_cache = {
                    "encoder": cur_feats,
                }
            else:
                cur_feats = self.reframe_cache["encoder"]
                x = self.encoder(cur_image, use_reframe=True)
                cur_feats[0] = x
        ...
        feature_outputs = self.depth_decoder(cur_feats, refresh_cache=refresh_cache)
