...
class BDDecoderPP(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=16, use_skips=True, use_reframe=False):
        super().__init__()
    ...
    def forward(self, input_features, refresh_cache=True):
        ...
        if self.use_reframe:
            if self.reframe_cache is not None:
                j = 1
                i = 0
                inputs = [self.convs[f"right_conv_{i}{j - 1}"](prev_outputs[i])]
                inputs += self.reframe_cache[f"{j}_a"]
                inputs += self.reframe_cache[f"{j}_b"]
                output = self.convs[f"in_conv_{i}{j}"](torch.cat(inputs, dim=1))

                j = 2
                inputs = [self.convs[f"right_conv_{i}{j - 1}"](output)]
                inputs += self.reframe_cache[f"{j}_a"]
                inputs += self.reframe_cache[f"{j}_b"]
                output = self.convs[f"in_conv_{i}{j}"](torch.cat(inputs, dim=1))
                ...
                for j in range(1, 5):
                    ...
                    for i in range(max_i, -1, -1):
                        inputs = [self.convs[f"right_conv_{i}{j - 1}"](prev_outputs[i])]
                        if i == 0:
                            self.reframe_cache[f"{j}_a"] = [upsample(self.convs[f"diag_conv_{i + 1}{j - 1}"](prev_outputs[i + 1]))]
                            inputs += self.reframe_cache[f"{j}_a"]
                        else:
                        ...
                        if i + j != 4:
                            if i == 0:
                                self.reframe_cache[f"{j}_b"] = [upsample(self.convs[f"up_conv_{i + 1}{j}"](outputs[-1]))]
                                inputs += self.reframe_cache[f"{j}_b"]
                            else:
