...
class OursNet(nn.Module):
     def __init__(self,is_train,lr_hidc=32,hr_hidc=32,mlpc=64,use_reframe=False) -> None:
        super(OursNet,self).__init__()

        # > REFRAME start
        self.use_reframe = use_reframe
        print(f"Using ReFrame: {self.use_reframe}")
     ...
     def set_hidden(self,hid,res):
        self.hidden=hid
        self.resimg=res
     def forward(self,cur_lr,cur_hr,prev_hr,mv,jitter,r,refresh_cache=True):
        #cur_lr:[img]
        ...
        # > REFRAME start
        if self.reframe_cache is not None and not refresh_cache:
            # Skip G-Buffer Feature Extraction and use cached features
            gbuffer_features = self.reframe_cache["hr"]
        else:
        ...
        # > REFRAME start
        if self.reframe_cache is not None and not refresh_cache:
            # Skip Temporal Feature Extraction
            pass
        else:
            if self.resimg!=None:
        ...
        if config.Pixelshuffle:
            # > REFRAME start
            # Baseline
            if not self.use_reframe:
                feature_fusion = self.imgConv(torch.cat([up_lr_features,temp_features,recon_hr_feat],dim=1))

            # Use cache
            elif self.reframe_cache is not None and not refresh_cache:
                # Use cached features for fusion
                feature_fusion = self.imgConv(torch.cat([up_lr_features, self.reframe_cache["temp"], recon_hr_feat], dim=1))
            ...
            self.resimg=self.lastConv(F.pixel_shuffle(ret,2)+F.pixel_shuffle(feature_fusion,2))
        else:
